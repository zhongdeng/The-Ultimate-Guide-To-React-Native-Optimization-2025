...
![通过 React Native DevTools 中的 "View Settings" 图标访问的通用分析器设置](../assets/4.png "通过 React Native DevTools 中的 "View Settings" 图标访问的通用分析器设置")

以及`Record why each component rendered while profiling`

![通过 React Native DevTools 的 "View Settings" 图标访问的分析器设置](../assets/5.png "通过 React Native DevTools 的 "View Settings" 图标访问的分析器设置")
...
!["Start profiling" 按钮](../assets/6.png "“Start profiling”按钮")
...
![显示 React Profiler 的图像，图中展示了 React 组件在分析会话中渲染和重新渲染的火焰图表示](../assets/7.png "显示 React Profiler 的图像，图中展示了 React 组件在分析会话中渲染和重新渲染的火焰图表示")
...
![显示 React Profiler 的图像，图中展示了 React 组件在分析会话中渲染和重新渲染的排名表示](../assets/8.png "显示 React Profiler 的图像，图中展示了 React 组件在分析会话中渲染和重新渲染的排名表示")
...
![相同流程的火焰图，但由于 memoization（记忆化），其中一个按钮未被更新](../assets/9.png "相同流程的火焰图，但由于 memoization（记忆化），其中一个按钮未被更新")
...
![Profiler输出显示长时间运行的 longRunningFunction，执行超过 12 秒](../assets/10.png "分析器输出显示长时间运行的 longRunningFunction，执行超过 12 秒")

如果 Chart（即火焰图）信息难以阅读，你可以尝试其他视图，比如 Heavy（自底向上），它可以让你按函数运行时间从长到短排序。

![同一分析信息的 "Heavy (自底向上)" 视图，longRunningFunction 位于顶部](../assets/11.png "同一分析信息的 "Heavy (自底向上)" 视图，longRunningFunction 位于顶部")
