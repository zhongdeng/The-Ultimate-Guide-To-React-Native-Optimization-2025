...
JavaScript 引擎（如 Hermes）以及其他依赖虚拟机来执行的解释型语言，都实现了一个叫作垃圾回收器（Garbage Collector，简称 GC）的特殊程序。顾名思义，它“回收垃圾”，这里的“垃圾”指的是那些应用运行时不再需要、可以释放出来供其他程序使用的内存地址。GC 会定期扫描已分配的对象，并判断哪些对象不再需要。一个有趣的小知识是：Hermes 的垃圾回收器被称为 Hades（冥王）。
